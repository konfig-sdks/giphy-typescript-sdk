...
import { readableStreamToString, GiphyError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new GiphyError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new GiphyError(e, e.response?.data)
                }
                if (innerError instanceof GiphyError) {
                    // Got "GiphyError" from the above try block
                    throw innerError;
                }
